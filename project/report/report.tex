...
Since we assume treated units cannot be further exposed, $\mathbf{Z} \times \mathbf{C} = 0$. If we could observe $\mathbf{C}$, then our work would be done, since we could partition the units into treatment, exposured, and control groups, and employ classic techniques to infer their differences in outcome. 
